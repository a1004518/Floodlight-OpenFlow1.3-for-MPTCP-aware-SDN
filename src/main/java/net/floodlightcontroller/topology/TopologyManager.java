...
ITopologyManagerBackend, ILinkDiscoveryListener, IOFMessageListener {
	...
	public Path getRoute(DatapathId src, OFPort srcPort, DatapathId dst, OFPort dstPort, U64 cookie) {
	...
	public Path getRoute(DatapathId src, OFPort srcPort, DatapathId dst, OFPort dstPort, U64 cookie,
		...
		TopologyInstance ti = getCurrentInstance();
		return ti.getRoute(null, srcPort, dst, dstPort, cookie);
	}
    
    private  void searchDfs(DatapathId current, DatapathId target, Stack<DatapathId> visited, int threshold){
        ...
        for(Link l:allLinks.get(current)){
            ...
            searchDfs(l.getDst(),target,nvisited,threshold);
	...
	public <T> int intersection(List<T> list1, List<T> list2) {
	...
	public synchronized ArrayList<Path> getRoutes(DatapathId srcDpid, DatapathId dstDpid,
			boolean tunnelEnabled, int k, boolean isDisjoint, String topo){
		 	ArrayList<Path> allRoutes = new ArrayList<Path>();
		    route.clear();
		    routesList.clear();
		    nvisited.clear();
		    linkCount=0;
		    this.searchDfs(srcDpid,dstDpid,nvisited,threshold);
		    for(Stack<NodePortTuple> r:routesList){
		        List<NodePortTuple> ports = new ArrayList<NodePortTuple>();
		        for(NodePortTuple np:r){
		            ports.add(np);
		        }
		        Path path = new Path(srcDpid, dstDpid);
		        path.setPath(ports);
		        allRoutes.add(path);
		    }
		  Collections.sort(allRoutes);
		  ArrayList<Path> ecmpRoutes = new ArrayList<Path>();
		  ArrayList<Path> selectedRoutes = new ArrayList<Path>();
		  int minLength = allRoutes.get(0).getPath().size();
		  for( Path r : allRoutes){
			  
		  if(r.getPath().size()==minLength)
			  ecmpRoutes.add(r);
		  else
			  break;
		  }
         Collections.shuffle(ecmpRoutes);
         
         if(k>0){
         // if we want a set of disjoint paths in the FatTree topology
         // calculate the disjoint paths from the set of shortest paths
		 if(isDisjoint==true && topo.equals("ft") ){
			  for( Path r : ecmpRoutes){  
			  List<NodePortTuple> used = new ArrayList<NodePortTuple>();
			  if(intersection(r.getPath(),used)==0){
				  selectedRoutes.add(r);
				  used.addAll(r.getPath());
			  }
			  if(selectedRoutes.size()>=k){
				  break;
			  }
			  }
			  
		 }
		 // if we want a set of dijoint paths in the Jellyfish topology
		 //calculate the disjoint paths from all the calculated paths
		 else if (isDisjoint==true && topo.equals("jelly")){
			 for( Path r : allRoutes){  
				  List<NodePortTuple> used = new ArrayList<NodePortTuple>();
				  if(intersection(r.getPath(),used)==0){
					  selectedRoutes.add(r);
		 ...
		 // if we want the k-shortest paths then select only the k shortest paths from allRoutes
		 else if(isDisjoint==false){
			 for(Path r : allRoutes){
				 selectedRoutes.add(r);
				 if(selectedRoutes.size()>=k){
					 break;
         ...
         //if k is not greater than zero then return only the shortest paths thus simulating ECMP path set
		 else{
			 Collections.shuffle(ecmpRoutes);
			  return ecmpRoutes;
			}
		  Collections.shuffle(selectedRoutes);
	...
	public synchronized ArrayList<Path> getRoutes(DatapathId srcDpid, DatapathId dstDpid,
	    ...
	    //v.clear();
	    ArrayList<Path> routes = new ArrayList<Path>();
	    route.clear();
	    routesList.clear();
	    nvisited.clear();
	    this.searchDfs(srcDpid,dstDpid,nvisited,threshold);
